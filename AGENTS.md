...
- Build execution plan (phases, gates): `~/.Codex/plans/reactive-squishing-dongarra.md`
...
- **Verify-feature gate after every feature** (skill: `.Codex/skills/verify-feature`).
  ...
  **Playwright** (scripted, reproducible gate) and **browser-harness** (live Chrome
  ...
  learning is always on (`BH_DOMAIN_SKILLS=1`, set in `.Codex/settings.local.json` env).
...
- **Subagents in parallel** (defs in `.Codex/agents/`: route-mapper, todo-collector,
  dep-auditor, researcher, reviewer). Bulk reading/scraping → Haiku/Sonnet; scoped code →
  Sonnet; architecture/correctness/review → Opus. All report back. In-session bulk → Codex
