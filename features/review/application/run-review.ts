...
export async function computeAndPersistFindings(
  ...
  const valid = await deps.validateCitations(collectPins(computed));
  const findings = applyResolvedPins(computed, valid);
  // Each FINDING that loses its pin counts, not each distinct section: two findings citing
  // the same missing section are two citations the user did not get.
  const dropped = computed.filter(
    (f) => f.mpep_section && !valid.has(f.mpep_section),
  ).length;
